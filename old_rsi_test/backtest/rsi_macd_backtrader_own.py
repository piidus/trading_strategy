...
OUTPUT_CSV_FILE = 'backtrader_backtest_summary_custom_sma.csv' # Updated file name
TRADE_DETAILS_CSV_FILE = 'backtrader_trade_details_custom_sma.csv' # Updated file name
PLOT_OUTPUT_DIR = 'backtrader_plots_custom_sma' # Updated directory name

# Moving Average Periods for Strategy 1 (SMA)
SMA_SHORT_PERIOD = 50
SMA_LONG_PERIOD = 200

# RSI Periods for Strategy 2
RSI_PERIOD = 14
RSI_MA_PERIOD = 21 # Moving Average period for RSI
...
# --- Custom SMA Indicator Class ---
class CustomSMA(bt.Indicator):
    """
    Calculates the Simple Moving Average (SMA) manually.
    """
    lines = ('sma',) # The name of the line produced by this indicator
    params = (('period', 20),) # Default period, can be overridden

    def __init__(self):
        # The 'self.data0' refers to the default data feed (usually the close price)
        # 'self.data' is an alias for 'self.data0' by default.
        # Ensure we have enough data points for the initial calculation
        self.addminperiod(self.p.period)

    def next(self):
        # 'self.data.get(size=self.p.period)' gets the last 'period' values
        # from the input data line (which is 'self.data0' by default).
        # We access the values using a list comprehension to ensure they are floats.
        
        # Check if enough data is available before calculating
        if len(self.data) >= self.p.period:
            # Calculate the sum of the last 'period' data points
            sum_of_values = sum(self.data.get(size=self.p.period))
            
            # Calculate the SMA
            current_sma = sum_of_values / self.p.period
            
            # Set the current value of our 'sma' line
            # self.lines.sma[0] is the current value of the 'sma' line
            self.lines.sma[0] = current_sma
        else:
            # If not enough data, set SMA to NaN (or None, depending on backtrader version)
            # This handles the warm-up period correctly
            self.lines.sma[0] = float('nan') # Explicitly set to NaN
...
class DualSMARSIStrategy(bt.Strategy):
    ...
    # --- Custom Indicator Functions (now using CustomSMA) ---
    def _add_sma(self, data_line, period, name_suffix=''):
        """Helper function to add a Custom Simple Moving Average indicator."""
        # Now instantiating our custom CustomSMA class
        return CustomSMA(data_line, period=period, plotname=f'SMA{period}{name_suffix}')

    def _add_rsi(self, data_line, period):
        """Helper function to add an RSI indicator."""
        # Still using bt.indicators.RSI as requested
        return bt.indicators.RSI(data_line, period=period, plotname=f'RSI{period}')
    # --- End Custom Indicator Functions ---
    ...
    def __init__(self):
        ...
        # SMA Indicators using our new custom helper function
        # Note: self.sma_s.sma[0] is used later to access the custom SMA's value
        self.sma_s = self._add_sma(self.dataclose, self.p.sma_short, '_Short')
        self.sma_l = self._add_sma(self.dataclose, self.p.sma_long, '_Long')
        
        # We need to explicitly access the 'sma' line from our CustomSMA indicator
        self.sma_cross = bt.indicators.CrossOver(self.sma_s.sma, self.sma_l.sma)

        # RSI and RSI-MA Indicators using the existing helper function
        self.rsi = self._add_rsi(self.dataclose, self.p.rsi_period)
        self.rsi_ma = self._add_sma(self.rsi, self.p.rsi_ma_period, '_RSI_MA') # SMA of RSI still uses CustomSMA
        self.rsi_cross = bt.indicators.CrossOver(self.rsi, self.rsi_ma.sma) # Note .sma for rsi_ma
        ...
        if order.status in [order.Completed]:
            ...
            if order.isbuy():
                ...
                if self.position.size < 0:
                    ...
                    transaction_details['Shares'] = abs(order.executed.size)
                    self.trade_log.append(transaction_details)
                else:
    ...
    def notify_trade(self, trade):
        ...
        if entry_trade_idx != -1:
            if 'Short Entry' in self.trade_log[entry_trade_idx]['Action']:
                self.trade_log[entry_trade_idx]['Action'] = 'BUY (Short Exit)'
            self.trade_log[entry_trade_idx]['Exit_Date'] = self.datas[0].datetime.date(0)
            self.trade_log[entry_trade_idx]['Exit_Price'] = trade.price
    ...
    def next(self):
        ...
        # Max period calculation must consider the SMA periods
        ...
        # Accessing the .sma line from our custom indicator
        current_sma_s = self.sma_s.sma[0]
        current_sma_l = self.sma_l.sma[0]
        current_rsi = self.rsi[0]
        current_rsi_ma = self.rsi_ma.sma[0] # Accessing .sma line from RSI's MA

        # Check if indicators have valid values (not NaN)
        # We need to explicitly check the values of the .sma line now
        if (pd.isna(current_sma_s) or pd.isna(current_sma_l) or
            pd.isna(current_rsi) or pd.isna(current_rsi_ma)):
            if self.p.debug_next:
                self.log(f"DEBUG: Indicators not ready/NaN. SMA_S={current_sma_s:.2f}, SMA_L={current_sma_l:.2f}, RSI={current_rsi:.2f}, RSI_MA={current_rsi_ma:.2f}. Skipping trade logic.")
            return
        ...
        if self.p.debug_next:
            ...
            self.log(f"DEBUG: SMA_S={current_sma_s:.2f}, SMA_L={current_sma_l:.2f}, SMA_Cross={sma_crossover_val}")
            self.log(f"DEBUG: RSI={current_rsi:.2f}, RSI_MA={current_rsi_ma:.2f}, RSI_Cross={rsi_crossover_val}")
            self.log(f"DEBUG: SMA_S > SMA_L: {current_sma_s > current_sma_l}, RSI > RSI_MA: {current_rsi > current_rsi_ma}")


        # --- Strategy Logic ---
        # Long Entry: SMA Short crosses above SMA Long AND RSI is currently above RSI MA
        long_entry_condition = (sma_crossover_val > 0) and (current_rsi > current_rsi_ma)

        # Short Entry: SMA Short crosses below SMA Long AND RSI is currently below RSI MA
        short_entry_condition = (sma_crossover_val < 0) and (current_rsi < current_rsi_ma)
    ...
    for scrip_symbol in SCRIPS:
        ...
        try:
            cerebro.plot(style='candlestick',
                         plotname=f'{scrip_symbol} Dual Custom SMA-RSI Crossover Backtest',
                         path=PLOT_OUTPUT_DIR,
                         filename=f'{scrip_symbol}_dual_custom_sma_rsi_crossover_backtest.html',
                         fmt='html',
                         iplot=False)
            print(f"Plot saved for {scrip_symbol} in {os.path.join(PLOT_OUTPUT_DIR, f'{scrip_symbol}_dual_custom_sma_rsi_crossover_backtest.html')}")

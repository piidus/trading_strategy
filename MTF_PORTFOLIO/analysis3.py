...
def sell_stock(row, holding: pd.DataFrame, signal='sell'):
    ...
    if stock_name in holding['stock'].values:
        ...
        if signal == 'sell':            
            ...
            # 30% of profit
            PER_STOCK_INVEST += round(((_profit * 0.90)/70), 2)
        elif signal == 'deliver':
            # only increse TOTAL_FUND
            TOTAL_FUND += ((today_price * quantity) * 2)
...
def average_stock(row, holding: pd.DataFrame, signal='avg'):
    ...
    if stock_name in holding['stock'].values :
        ...
        if signal == 'avg':
            ...
            quantity  =  int(PER_STOCK_INVEST / today_price) + stock_data['quantity']
            TOTAL_FUND -= ((today_price * quantity)*2)
...
def deliver_stock(row, holding: pd.DataFrame, date, signal='deliver'):
    global TOTAL_FUND
    stock_name = row['ShortName']
    # find average price and quantity
    if stock_name in holding['stock'].values:
        stock_data = holding[holding['stock'] == stock_name].iloc[0]
        avg_price = stock_data['avg_price']
        quantity = stock_data['quantity']
        ...
        no_of_holds = count_holding()
        # TOTAL_FUND -=(avg_price * (quantity * 2/3))
        TOTAL_FUND -= ((avg_price * quantity) * 2)

        # UPDATE HOLDINGS
        update_holding(stock_name, {'signal': signal, 'date': date})  
    ...
    print(f"Delivered {stock_name} at {date} - holding {count_holding()} stocks ...........................")
    return holding
...
def portfolio_analysis(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for date in unique_dates:
        ...
        for _, row in daily_data.iterrows():
            ...
            if stock_name not in holding['stock'].values and temp_hold > 0 and len(holding) < 70:
                ...
                if today_price >= avg_price * 1.03:
                    if holding.loc[holding['stock'] == stock_name, 'signal'].values[0] == 'deliver':
                        holding = sell_stock(row, holding, signal='deliver')
                    else:
                ...
                # elif (pd.to_datetime(date) - pd.to_datetime(holding['date'].values[0])).days >= 41 and holding['signal'].values[0] != 'deliver':
                ...
                elif (pd.to_datetime(date) - stock_date).days >= 41 and stock_signal != 'deliver':
                    if TOTAL_FUND >= (avg_price * __quantity):
                        holding = deliver_stock(row, holding, date)
                    else:
                        print(f"MISS DELIVER: Date: {date}, Total Funds: {TOTAL_FUND}, Total Holdings: {len(holding)}")
                elif today_price <= avg_price * 0.9  and stock_signal != 'deliver':
                    ...
                    # CHECK FUND AVAILABILITY                    
                    if TOTAL_FUND >= (avg_price * __quantity):
                        holding = average_stock(row, holding, signal='deliver')
                    else:
                        print(f"MISS AVERAGE: Date: {date}, Total Funds: {TOTAL_FUND}, Total Holdings: {len(holding)}")
                # elif stock_signal == 'deliver' and (pd.to_datetime(date) - stock_date).days >= 41:
                #     # holding = deliver_stock(row, holding)
                #     print(f"DELIVERED: Date: {date}, Total Funds: {TOTAL_FUND}, Total Holdings: {len(holding)}")

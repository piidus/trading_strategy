...
def get_data() -> list:
    ...
    """
    etf_list = pd.read_csv('MTF_PORTFOLIO/etf_list_yahoo.csv')
...
def data_read(etf_list: str) -> pd.DataFrame:
    ...
    for etf in etf_list:
        # print(etf)
        try:
            df = pd.read_csv(f'MTF_PORTFOLIO/data_yahoo/{etf}.csv')
            df = data_clean(df)
            # print(df.head())
        except Exception as e:
...
if __name__ == '__main__':
    ...
    df.to_csv('MTF_PORTFOLIO/data_yahoo/total_etf_data.csv', index=False)
    # print(df.head())
    df = pd.read_csv('MTF_PORTFOLIO/data_yahoo/total_etf_data.csv')
    # print(df.head())
    data:pd.DataFrame = portfolio_analysis(df)
    data.to_csv('MTF_PORTFOLIO/portfolio_analysi_yahoo.csv', index=False)
